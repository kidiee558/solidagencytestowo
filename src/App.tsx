          ...
          <div className="text-left lg:col-span-7 xl:col-span-6 w-full lg:pl-12 xl:pl-20">
            <h1 className="font-bold text-white tracking-tight mb-6 lg:mb-8 leading-[1.1] flex flex-col gap-1 lg:gap-2">
              <span className="flex flex-col items-start">
                ...
                <span className="text-xl sm:text-2xl lg:text-3xl font-bold font-syne text-white tracking-wider [text-shadow:0_2px_4px_rgba(0,0,0,0.3)] mt-2 block">
                  ZARABIAJ OD <span className="relative"><span className="absolute top-[-0.01em] left-[0.02em] text-white">JUTRA</span><span className="relative text-[#8d32fc]">JUTRA</span></span>.
                </span>
              </span>
            </h1>
            <p className="text-xs sm:text-base lg:text-lg text-white mb-8 lg:mb-12 leading-relaxed max-w-2xl font-normal [text-shadow:0_2px_4px_rgba(0,0,0,0.5)]">
              Zamiast lania wody, dostarczamy unikalne <strong>strony WWW</strong>, szybkie <strong>sklepy internetowe</strong> i skuteczne <strong>kampanie Google & Meta Ads</strong> w <strong>24-godzinnym sprincie</strong>. Rezultat? <strong>Solid Agency</strong>. Budujemy <strong>dochodowe biznesy</strong> w czasie, gdy inni dopiero odpisują na maila.
            </p>
          ...
          <div className="lg:col-span-7 text-left lg:pl-12 xl:pl-20">
            ...
            <div className="space-y-6">
              <p className="text-base md:text-lg text-slate-700 leading-relaxed font-medium font-sans">
                Każda godzina Twojego wahania to połączenie odebrane przez kogoś innego. Przekształcamy Twoje pomysły w <strong className="font-bold text-slate-900">profesjonalną maszynę do zdobywania klientów</strong> która pracuje 24/7. Przestań być obserwatorem – zostań liderem w swojej branży.
              </p>
              <p className="text-base md:text-lg text-slate-700 leading-relaxed font-medium font-sans">
                Koniec z planowaniem i czekaniem na cud. Przejmij zlecenia które teraz zgarnia konkurencja i zacznij dominować tam gdzie inni wciąż stoją w miejscu. Postaw na rozwiązanie które zamiast generować koszty zacznie w końcu <strong className="font-bold text-slate-900">przynosić realne pieniądze</strong>.
              </p>
            </div>
          </div>
...
function ExpertiseSection() {
  return (
    <div className="pt-24 pb-32 sm:pb-40 lg:pb-48 relative z-20" id="ekspertyza">
      <div className="w-full max-w-[1800px] mx-auto px-4 sm:px-10 lg:px-16 relative z-10">
        <div className="text-center mb-16">
          <span className="text-[#8d32fc] font-bold text-xs tracking-widest uppercase mb-3 block font-syne">Nasza Oferta</span>
                ...
                <div className="mt-auto w-full flex flex-col sm:flex-row items-center sm:justify-end gap-3">
                  {(service.id === 'www' || service.id === 'ecommerce') && (
                    <div className="w-full sm:w-auto flex items-center justify-center px-6 py-3 rounded-full text-xs sm:text-sm font-extrabold bg-[#8d32fc]/10 text-[#8d32fc] border border-[#8d32fc]/20 shadow-sm whitespace-nowrap">
                      {service.id === 'www' ? 'od 1 000 PLN' : 'od 2200 PLN'}
                    </div>
                  )}
...
const MinimalArrow = ({ direction }: { direction: 'right' | 'down' | 'left' }) => {
  ...
  return (
    ...
    >
      <svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" className="w-full h-full overflow-visible">
        <rect x="0" y="11" width="2" height="2" stroke="#8d32fc" strokeWidth="1" />
        <motion.line 
          x1="2" y1="12" x2="21" y2="12" 
          stroke="#8d32fc" strokeWidth="1" strokeDasharray="2 2"
          animate={{ strokeDashoffset: [0, -4] }}
          transition={{ repeat: Infinity, duration: 1, ease: "linear" }}
        />
        <path d="M18 9L22 12L18 15" stroke="#8d32fc" strokeWidth="1" strokeLinecap="round" strokeLinejoin="round" />
      </svg>
    </div>
  );
};
...
function ProcessSection() {
  const steps = [
    { 
      ...
      desc: 'Nie zgadujemy. Analizujemy Twój biznes, konkurencję i grupę docelową. Tworzymy precyzyjny plan działania, który ma jeden cel: maksymalizację Twoich zysków.',
      icon: <Search className="w-6 h-6 sm:w-8 sm:h-8 text-[#8d32fc]" />
    },
    { 
      title: 'Projektowanie UI/UX', 
      desc: 'Tworzymy makiety i design, który nie tylko wygląda obłędnie, ale przede wszystkim sprzedaje. Skupiamy się na intuicyjnej nawigacji i ścieżce konwersji.',
      icon: <Target className="w-6 h-6 sm:w-8 sm:h-8 text-[#8d32fc]" />
    },
    { 
      title: 'Wdrożenie & Development', 
      desc: 'Piszemy czysty, błyskawicznie działający kod. Optymalizujemy każdy obrazek i skrypt, aby Twoja strona ładowała się w ułamku sekundy.',
      icon: <Zap className="w-6 h-6 sm:w-8 sm:h-8 text-[#8d32fc]" />
    },
    { 
      title: 'Skalowanie & Optymalizacja', 
      desc: 'Strona to dopiero początek. Odpalamy kampanie, analizujemy dane i ciągle optymalizujemy lejek sprzedażowy, by wycisnąć z niego 200% normy.',
      icon: <TrendingUp className="w-6 h-6 sm:w-8 sm:h-8 text-[#8d32fc]" />
    },
  ];
    ...
    >
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="flex flex-col lg:flex-row gap-16 lg:gap-8 items-center lg:items-start justify-between">
          ...
          {/* Lewa strona - Tekst */}
          <div className="w-full lg:w-5/12 lg:sticky lg:top-32 text-center lg:text-left">
            <span className="text-[#8d32fc] font-bold text-xs tracking-widest uppercase mb-3 block font-syne">Nasz Proces</span>
            <h2 className="text-4xl md:text-5xl lg:text-6xl font-bold mb-6 tracking-tight font-syne text-white">
              <span className="whitespace-nowrap">Jak dowieziemy</span> <br className="hidden lg:block" />
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#8d32fc] to-purple-400 whitespace-nowrap">Twój sukces?</span>
            </h2>
            <p className="text-lg sm:text-xl text-slate-400 max-w-xl mx-auto lg:mx-0 font-medium mb-8">
              Zero zgadywania. 100% danych i sprawdzonych schematów. Przeprowadzimy Cię przez przejrzysty proces, który zamienia pomysły w zyskowne rozwiązania.
            </p>
          </div>

          {/* Prawa strona - Grid 2x2 */}
          <div className="w-full lg:w-1/2">
            <div className="grid grid-cols-2 gap-4 sm:gap-6 lg:gap-12 relative z-40">
              {steps.map((step, i) => {
                let orderClass = '';
                if (i === 0) orderClass = 'order-1';
                if (i === 1) orderClass = 'order-2';
                if (i === 2) orderClass = 'order-4';
                if (i === 3) orderClass = 'order-3';
                  ...
                  <div key={i} className={`relative flex ${orderClass}`}>
                    ...
                    >
                      <div className="flex justify-between items-start mb-3 sm:mb-6">
                        ...
                        <div className="text-3xl sm:text-5xl font-syne font-bold text-white/5 group-hover:text-[#8d32fc]/20 transition-colors duration-300">
                          0{i + 1}
                        </div>
                      </div>
                      <div className="mt-auto">
                        <h3 className="text-sm sm:text-xl lg:text-2xl font-syne font-bold text-white mb-2 sm:mb-3 group-hover:text-[#8d32fc] transition-colors duration-300 leading-tight">
                          {step.title}
                        </h3>
                        <p className="text-[10px] sm:text-sm text-slate-400 leading-relaxed line-clamp-3 sm:line-clamp-none">
                          {step.desc}
                        </p>
                      </div>
                    </motion.div>
                    ...
                    {/* Strzałki */}
                    {i === 0 && (
                      <div className="absolute top-1/2 right-0 translate-x-full -translate-y-1/2 w-4 sm:w-6 lg:w-12 h-4 sm:h-6 lg:h-12 z-10">
                        <MinimalArrow direction="right" />
                      </div>
                    )}
                    {i === 1 && (
                      <div className="absolute bottom-0 left-1/2 -translate-x-1/2 translate-y-full w-4 sm:w-6 lg:w-12 h-4 sm:h-6 lg:h-12 z-10">
                        <MinimalArrow direction="down" />
                      </div>
                    )}
                    {i === 2 && (
                      <div className="absolute top-1/2 left-0 -translate-x-full -translate-y-1/2 w-4 sm:w-6 lg:w-12 h-4 sm:h-6 lg:h-12 z-10">
