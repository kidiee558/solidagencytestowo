...
function Navbar({ 
  ...
  return (
    <nav className={`fixed top-6 left-0 w-full max-w-[100vw] z-50 px-4 pointer-events-none flex flex-col ${isStartProject ? 'items-start' : 'items-end'} xl:block`}>
...
function HeroContent({ onContactClick, onOfferClick }: { onContactClick: () => void, onOfferClick?: () => void }) {
  ...
  return (
    <div className="relative min-h-[100vh] flex flex-col items-start justify-center pt-32 lg:pt-28 xl:pt-32 pb-16 lg:pb-12 overflow-hidden z-0">
          ...
          <div className="text-left w-full">
            <div className="flex flex-col items-start w-full">
              <h1 
                ...
                aria-label={lang === 'PL' 
                  ? "SOLIDNY MARKETING - STRONY INTERNETOWE & SKLEPY INTERNETOWE / KAMPANIE GOOGLE ADS & META ADS / POZYCJONOWANIE SEO" 
                  : "SOLID MARKETING - WEBSITES & E-COMMERCE STORES / GOOGLE ADS & META ADS CAMPAIGNS / SEO POSITIONING"
                }
              >
                <span className="flex flex-col items-start w-full" aria-hidden="true">
                  <div 
                    className="text-[clamp(7.5rem,22vw,32rem)] sm:text-[clamp(8.5rem,-2.6229508197rem+28.8524590164vw,32rem)] font-humane font-black text-white leading-[0.75] uppercase tracking-[0.005em] relative text-left w-full"
                    style={{ marginBottom: 'calc(-0.04em + 10px)' }}
                  >
                    <span className="flex flex-col w-full">
                      <span className="flex justify-start items-end w-full gap-1 sm:gap-3 lg:gap-4 relative">
                        <span className="whitespace-nowrap text-left leading-[0.75]">{lang === 'PL' ? 'SOLIDNY' : 'SOLID'}</span>
                        <div className="absolute right-[2vw] sm:relative sm:right-auto text-left flex flex-col items-start justify-end bottom-[2.5vw] sm:bottom-auto sm:mb-[2.2vw] lg:mb-[1.7vw] xl:mb-[2vw]">
                          <div 
                            className="font-inter-tight text-slate-200 uppercase whitespace-nowrap text-left"
                            style={{ 
                              fontSize: 'clamp(0.3rem, 1.1vw, 1.5rem)',
                          ...
                          >
                            {lang === 'PL' ? (
                              <>
                                ...
                                KAMPANIE GOOGLE ADS & META ADS / <br />
                                POZYCJONOWANIE SEO
                              </>
                            ) : (
                              <>
                                WEBSITES & E-COMMERCE STORES / <br />
                                GOOGLE ADS & META ADS CAMPAIGNS / <br />
                                SEO POSITIONING
                              </>
                            )}
                          </div>
                  ...
                  <div className="flex flex-col sm:flex-row justify-between items-start w-full -mt-2 sm:mt-0">
                    <div 
                      className="flex flex-col items-start gap-6 pb-4 sm:pb-[2vw] w-full sm:w-auto order-2 sm:order-1 mt-5 sm:mt-4 lg:mt-6 xl:mt-8"
                    >
                      <div className="w-full">
                        <div className="w-[calc(100%-190px)] sm:w-20 h-[1px] bg-white/30 mb-4" />
                        <div 
                          className="font-inter-tight text-slate-200 uppercase text-left break-words max-w-[45ch]"
                          style={{ 
                        ...
                        >
                          {lang === 'PL' ? 'AUTORSKI SYSTEM SKALOWANIA SPRZEDAŻY. ARCHITEKTURA STRON NASTAWIONA NA ZYSK.' : 'PROPRIETARY SALES SCALING SYSTEM. WEBSITE ARCHITECTURE FOCUSED ON PROFIT.'}
                        </div>
                      </div>
                      <div className="flex justify-center sm:justify-start w-full sm:w-auto gap-3 shrink-0">
                        <button 
                          onClick={onContactClick}
                    ...
                    <div className="flex flex-col items-end w-full sm:w-auto order-1 sm:order-2 relative">
                      <div 
                        className="text-[clamp(7.5rem,22vw,32rem)] sm:text-[clamp(8.5rem,-2.6229508197rem+28.8524590164vw,32rem)] font-humane font-black text-white leading-[0.75] uppercase tracking-[0.005em] relative text-right w-full"
                        style={{ paddingTop: 'calc(0.04em + 10px)' }}
                      >
                        <span className="whitespace-nowrap">{lang === 'PL' ? 'MARKETING' : 'MARKETING'}</span>
                      </div>
                      <div 
                        className="font-inter-tight text-slate-400 uppercase break-words text-right absolute sm:relative top-full sm:top-auto right-0 mt-4 sm:mt-0"
                        style={{ 
...
function ExpertiseSection({ onWebsitesClick }: { onWebsitesClick: () => void }) {
  const { lang } = useLanguage();
  return (
      ...
      <div className="w-full max-w-[1800px] mx-auto px-4 sm:px-10 lg:px-16 relative z-10">
        <div className="flex flex-col lg:flex-row items-center gap-12 lg:gap-24">
          ...
          <div className="w-full lg:w-[35%] text-center lg:text-left px-4 sm:px-0">
            ...
            >
              {lang === 'PL' ? (
                <><span className="text-shimmer font-semibold inline-block">Inwestuj w wynik,</span> zamiast samej obecności.</>
              ) : (
                <><span className="text-shimmer font-semibold inline-block">Invest in results,</span> instead of just presence.</>
              )}
            </h2>
            <p 
            ...
            >
              {lang === 'PL' ? 'Estetyka premium to w naszym wydaniu precyzyjna inżynieria konwersji. Wykorzystujemy zaawansowaną architekturę, aby zamienić ruch z kampanii w realny kapitał Twojej firmy, eliminując błędy typowe dla standardowych wdrożeń. Dostarczamy kompletny fundament: od unikalnego wizerunku, po narzędzia automatyzujące skalowanie zysków.' : 'Premium aesthetics in our edition is precise conversion engineering. We use advanced architecture to turn campaign traffic into real capital for your company, eliminating errors typical of standard implementations. We provide a complete foundation: from a unique image to tools automating profit scaling.'}
            </p>
            ...
            <div className="mt-10 flex flex-col sm:flex-row items-center gap-6 lg:justify-end justify-center">
              ...
              <button 
                ...
                className="px-6 py-3 rounded-full border border-white/20 hover:border-white/40 bg-white/5 hover:bg-white/10 text-white font-medium transition-all flex items-center gap-2 group text-sm sm:text-base"
              >
...
const getProjectsData = (lang: 'PL' | 'ENG') => [
  {
    id: '01',
    location: lang === 'PL' ? 'PRZYKŁAD' : 'EXAMPLE',
  ...
  },
  {
    id: '02',
    location: lang === 'PL' ? 'PRZYKŁAD' : 'EXAMPLE',
    title: lang === 'PL' ? 'Rozbudowana strona firmowa' : 'Extended Corporate Website',
  ...
  },
  {
    id: '03',
    location: lang === 'PL' ? 'PRZYKŁAD' : 'EXAMPLE',
    title: lang === 'PL' ? 'Dedykowany system' : 'Dedicated System',
  ...
  {
    id: '05',
    location: lang === 'PL' ? 'WIZUALIZACJA 3D' : '3D VISUALIZATION',
        ...
        <div className="sticky top-0 h-screen flex w-full overflow-hidden">
          ...
          <div className="w-[35%] h-full bg-black flex flex-col justify-between p-8 lg:p-12 xl:p-16 relative border-r border-white/10 z-20">
            <div className="flex flex-col items-start pt-10 flex-grow">
              ...
              <div className="text-white text-[clamp(1.25rem,2vw,1.5rem)] mt-4 font-inter-tight">{lang === 'PL' ? 'Ostatnie Projekty' : 'Latest Projects'}</div>
              ...
              <div className="mt-8">
                <AnimatePresence mode="wait">
                  ...
                  >
                    ...
                    <h3 className="text-white text-[clamp(2.5rem,4vw,3.75rem)] font-inter-tight font-light leading-none">{projectsData[activeIndex].title}</h3>
                ...
                {projectsData.map((p, i) => (
                  <div key={i} className="w-full h-1/5 flex items-center justify-center p-4">
                    {p.videoUrl ? (
                      ...
                      <img src={p.fgImage} className={`w-full object-cover rounded-xl shadow-2xl h-auto max-h-[75vh]`} />
                    )}
                  </div>
                ))}
              </motion.div>
      ...
      <div className="md:hidden bg-black text-white pt-6 pb-20 px-4">
        <div className="flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-3 mt-2">
            ...
            <h3 className="text-4xl font-inter-tight font-light leading-none">{projectsData[mobileIndex].title}</h3>
    ...
    <>
      <section className="bg-white pb-12 lg:pb-24 px-6 lg:px-[6.25rem]">
        ...
        >
          <img 
            src={imageUrl} 
            alt="System Skalowania Solid Agency" 
...
function LatestNews({ onPoradnikClick, onArticleClick }: { onPoradnikClick: () => void, onArticleClick: (id: string) => void }) {
  ...
  const scroll = (direction: 'left' | 'right') => {
    if (viewportRef.current) {
      const slides = viewportRef.current.querySelectorAll('.slider-news__slide');
      ...
      const slide = slides[newIndex] as HTMLElement;
      const viewport = viewportRef.current;
      
      // Calculate scroll position to center the slide
      const scrollLeft = slide.offsetLeft - (viewport.offsetWidth - slide.offsetWidth) / 2;
    ...
    <section className="latest-news py-24 bg-white relative overflow-hidden" id="latest-news">
      <div className="w-full relative z-10 px-4 sm:px-6 lg:px-8">
        <h2 className="text-[clamp(3rem,8vw,5.5rem)] font-inter-tight font-normal text-black mb-12 tracking-tight leading-[0.9] sm:leading-tight">
          {lang === 'PL' ? (
            <>
              Nasz<span className="sm:hidden"><br /></span><span className="hidden sm:inline"> </span>Poradnik
            </>
          ) : (
            <>
              Our<span className="sm:hidden"><br /></span><span className="hidden sm:inline"> </span>Guide
            </>
          )}
        </h2>
        ...
        <div ref={containerRef} className="slider-news" style={{ 
          ...
          <div ref={viewportRef} className="slider-news__viewport" style={{ overflowX: 'auto', scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
            <div className="slider-news__container flex items-start touch-pan-y" style={{ transform: 'translate3d(0px, 0px, 0px)', marginLeft: 'calc(1rem * -1)' }}>
              {news.map((item, idx) => (
                <div key={idx} className="slider-news__slide min-w-0 pl-[1rem]" style={{ flex: `0 0 ${100 / cardsPerRow}%` }}>
                  <a href={item.link} onClick={(e) => { e.preventDefault(); onArticleClick(item.id); }} className="news-card block rounded-[20px] overflow-hidden flex flex-col bg-[#000] text-[#fff] border border-[rgba(255,255,255,0.25)] h-[24.375rem] select-none group relative">
                    ...
                    <div className="news-card__content p-[2.25rem] pb-0 flex flex-col justify-between h-full">
                      <h3 
                        className="font-inter-tight font-[300] tracking-[0.005em] line-clamp-3"
                        style={{ 
    ...
    <div className="relative bg-[#f8fafc] font-sans text-slate-900 selection:bg-[#8d32fc]/20 selection:text-[#8d32fc]">
        ...
        <main className="relative">
          ...
          <div className="relative z-10">
            <div className="relative z-10 bg-black overflow-hidden">
              <ExpertiseSection onWebsitesClick={() => handleNavigate('websites-offer')} />
            </div>
      ...
      {currentView === 'about' && (
        <AboutPage 
          ...
          onOfferClick={() => handleNavigate('ecommerce-offer')}
          onWebsitesClick={() => handleNavigate('websites-offer')}
        />
      )}
