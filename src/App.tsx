...
import { OnePageOfferPage } from './components/OnePageOfferPage';
          ...
          >
            ...
            <div className="relative p-6 sm:p-8 z-10 flex flex-col gap-6">
              {isSubmitted ? (
                ...
                <>
                  <div className="pr-8">
                    <h2 className="text-2xl sm:text-3xl font-inter-tight font-medium text-white mb-2 tracking-tight">
                  ...
                  <form className="flex flex-col gap-5" onSubmit={handleSubmit}>
                    <div className="flex flex-col gap-1.5">
                      <label className="text-xs font-semibold text-slate-400 uppercase tracking-wider font-inter-tight">{lang === 'PL' ? 'Imię i Nazwisko' : 'Full Name'}</label>
                      <input 
                        type="text" 
                        value={name}
                    ...
                    <div className="flex flex-col gap-1.5">
                      <label className="text-xs font-semibold text-slate-400 uppercase tracking-wider font-inter-tight">{lang === 'PL' ? 'Adres email / Numer telefonu' : 'Email address / Phone number'}</label>
                      <input 
                        type="text" 
                        value={contactInfo}
                    ...
                    <div className="relative flex flex-col gap-1.5">
                      <label className="text-xs font-semibold text-slate-400 uppercase tracking-wider font-inter-tight">{lang === 'PL' ? 'Usługa' : 'Service'}</label>
                      <button
                        type="button"
                    ...
                    <div className="flex flex-col gap-1.5">
                      <label className="text-xs font-semibold text-slate-400 uppercase tracking-wider font-inter-tight">{lang === 'PL' ? 'Twoja wiadomość' : 'Your message'}</label>
                      <textarea 
                        rows={4}
                        value={message}
                    ...
                    <button 
                      ...
                      className="w-full bg-[#c9c0e8] hover:bg-[#b8a1ff] text-black rounded-xl py-3.5 font-semibold transition-all shadow-lg flex items-center justify-center gap-2 mt-2 text-sm font-inter-tight disabled:opacity-70"
                    >
...
const getServicesData = (lang: 'PL' | 'ENG') => [
  {
    id: 'www',
    title: lang === 'PL' ? 'Strony internetowe WWW' : 'Websites',
    description: lang === 'PL' ? 'Od prostych stron One-Page po skomplikowane projekty z unikalnym designem. Kodujemy od zera bez szablonów, budując zaufanie od pierwszego kliknięcia.' : 'From simple One-Page sites to complex projects with unique design. We code from scratch without templates, building trust from the first click.',
    ...
    features: lang === 'PL' ? [
      ...
      'Business card websites (One Page)',
      'Extensive corporate portals',
      'Sales Landing Pages',
  ...
  {
    id: 'ecommerce',
    title: lang === 'PL' ? 'Sklepy E-commerce' : 'E-commerce Stores',
    description: lang === 'PL' ? 'Od szybkich sklepów po zaawansowane systemy e-commerce. Budujemy intuicyjne ścieżki zakupu, które zatrzymują klienta w koszyku aż do finalizacji zamówienia.' : 'From fast stores to advanced e-commerce systems. We build intuitive purchasing paths that keep the customer in the cart until the order is finalized.',
    ...
    features: lang === 'PL' ? [
      ...
      'WooCommerce / PrestaShop stores',
      'Allegro and ERP integrations',
  ...
  {
    id: 'google-ads',
    title: lang === 'PL' ? 'Reklamy Google' : 'Google Ads',
    description: lang === 'PL' ? 'Zamiast przepalać budżet na przypadkowe frazy, skupiamy się na intencjach zakupowych. Sprawiamy, że Twoja oferta pojawia się dokładnie tam, gdzie klienci szukają konkretnych rozwiązań.' : 'Instead of burning budget on random phrases, we focus on purchasing intentions. We make your offer appear exactly where customers are looking for specific solutions.',
  ...
  {
    id: 'meta-ads',
    title: lang === 'PL' ? 'Reklamy Meta' : 'Meta Ads',
    description: lang === 'PL' ? 'Tworzymy reklamy, których nie da się pominąć – zatrzymują wzrok i zmuszają do działania. Zamieniamy przypadkowy scroll w konkretne zapytania i realny zysk Twojej marki.' : 'We create ads that cannot be ignored – they catch the eye and force action. We turn a random scroll into specific inquiries and real profit for your brand.',
    ...
    features: lang === 'PL' ? [
      ...
      'Lead Generation',
      'Sales scaling',
      'Video/graphic ad creatives'
    ]
  },
  {
    id: 'seo',
    title: lang === 'PL' ? 'Pozycjonowanie SEO' : 'SEO Optimization',
    description: lang === 'PL' ? 'Zamiast płacić za każde kliknięcie, sprawiamy, że klienci sami trafiają na Twoją stronę. Budujemy trwały zasięg, który z każdym miesiącem obniża Twoje koszty marketingu.' : 'Instead of paying for every click, we make customers find your website themselves. We build lasting reach that lowers your marketing costs every month.',
    ...
    features: lang === 'PL' ? [
      ...
      'Content optimization (SXO)'
    ]
  },
  {
    id: 'strategy',
    title: lang === 'PL' ? 'Copywriting & Strategia' : 'Copywriting & Strategy',
    description: lang === 'PL' ? 'Łączymy mocny design z tekstami, które zapadają w pamięć. Projektujemy treści wybijające się z nudnego tła, budując Twój autorytet i realną przewagę nad konkurencją.' : 'We combine strong design with memorable texts. We design content that stands out from the boring background, building your authority and real advantage over the competition.',
...
const getPackagesData = (lang: 'PL' | 'ENG') => [
  {
    title: lang === 'PL' ? 'Pakiet "Szybki Start"' : '"Quick Start" Package',
    subtitle: lang === 'PL' ? 'Strona + Google Ads' : 'Website + Google Ads',
    desc: lang === 'PL' ? 'Idealny dla nowych firm, które chcą szybko zaistnieć w sieci i zacząć pozyskiwać pierwszych klientów.' : 'Ideal for new companies that want to quickly establish a presence on the web and start acquiring their first customers.',
    price: lang === 'PL' ? 'od 3 500 PLN' : 'from 3 500 PLN',
    features: lang === 'PL' ? ['Strona Wizytówka', 'Konfiguracja Google Ads', 'Szybki start (wdrożenie)', 'Analiza konkurencji'] : ['Business Card Website', 'Google Ads Configuration', 'Quick start (implementation)', 'Competitor analysis']
  },
  {
    title: lang === 'PL' ? 'Pakiet "E-commerce Scale"' : '"E-commerce Scale" Package',
    subtitle: lang === 'PL' ? 'Sklep + Meta Ads + Content' : 'Store + Meta Ads + Content',
    desc: lang === 'PL' ? 'Kompleksowe rozwiązanie dla sklepów, które chcą zwiększyć sprzedaż i zbudować rozpoznawalną markę.' : 'A comprehensive solution for stores that want to increase sales and build a recognizable brand.',
    price: lang === 'PL' ? 'od 8 500 PLN' : 'from 8 500 PLN',
    features: lang === 'PL' ? ['Sklep Internetowy', 'Kampania Meta Ads', 'Zdjęcia produktów', 'Kompleksowe wdrożenie', 'Automatyzacja'] : ['E-commerce Store', 'Meta Ads Campaign', 'Product photos', 'Comprehensive implementation', 'Automation']
  },
  {
    title: lang === 'PL' ? 'Abonament "Full Growth"' : '"Full Growth" Subscription',
    subtitle: lang === 'PL' ? 'Zewnętrzny dział marketingu' : 'External marketing department',
    desc: lang === 'PL' ? 'Stała opieka nad SEO, kampaniami Ads oraz regularna produkcja contentu. Twoja firma ma zewnętrzny dział marketingu.' : 'Constant care for SEO, Ads campaigns and regular content production. Your company has an external marketing department.',
    price: lang === 'PL' ? 'Indywidualnie' : 'Individually',
    features: lang === 'PL' ? ['Stała opieka SEO', 'Zarządzanie kampaniami Ads', 'Produkcja wideo', 'Dedykowany opiekun', 'Raportowanie'] : ['Constant SEO care', 'Ads campaigns management', 'Video production', 'Dedicated account manager', 'Reporting']
  }
];
...
function Navbar({ onContactClick, onStartProjectClick, onOfferClick, onHomeClick, onPoradnikClick, onAboutClick, onWorkClick }: { onContactClick: () => void, onStartProjectClick?: () => void, onOfferClick?: (type?: string) => void, onHomeClick: () => void, onPoradnikClick: () => void, onAboutClick?: () => void, onWorkClick?: () => void }) {
  ...
  return (
    <nav className="fixed top-6 left-0 right-0 z-50 px-4 pointer-events-none flex flex-col items-end xl:block">
      <div className="w-full flex justify-center relative">
              ...
              <div>
                ...
                <button onClick={() => { onStartProjectClick ? onStartProjectClick() : onContactClick(); setIsOpen(false); }} className="bg-[#d1cce8] p-5 sm:p-10 rounded-[1.5rem] sm:rounded-[2rem] flex flex-col justify-between h-36 sm:h-48 relative text-left group overflow-hidden flex-1">
                  <div className="relative z-10">
                    <span className="text-[8px] sm:text-[10px] font-bold text-slate-500 uppercase tracking-[0.2em] sm:tracking-[0.3em] mb-1 sm:mb-2 block">{t('nav.contact')}</span>
                    <span className="text-2xl sm:text-5xl font-black text-slate-900 leading-none tracking-tighter uppercase font-bebas" style={{ letterSpacing: '0.1em' }} dangerouslySetInnerHTML={{ __html: t('nav.start_project').replace(' Z ', ' Z<br/>') }}></span>
                  </div>
                ...
                <button onClick={() => { onContactClick(); setIsOpen(false); }} className="bg-white p-5 sm:p-10 rounded-[1.5rem] sm:rounded-[2rem] flex flex-col justify-between h-36 sm:h-48 relative text-left group overflow-hidden flex-1">
                  <div className="relative z-10">
                    <span className="text-[8px] sm:text-[10px] font-bold text-slate-400 uppercase tracking-[0.2em] sm:tracking-[0.3em] mb-1 sm:mb-2 block">{t('nav.contact')}</span>
                    <span className="text-2xl sm:text-5xl font-black text-slate-900 leading-none tracking-tighter uppercase font-bebas" style={{ letterSpacing: '0.1em' }} dangerouslySetInnerHTML={{ __html: t('nav.say_hello').replace(', ', ',<br/>') }}></span>
                  </div>
                  <div className="absolute top-1/2 -translate-y-1/2 right-2 sm:right-4 w-20 h-20 sm:w-32 sm:h-32 flex items-center justify-center group-hover:scale-110 transition-transform duration-500">
        ...
        <div className="hidden xl:block absolute right-0 top-3">
          <LanguageSwitcher />
        </div>
      </div>
...
function PhoneMockup() {
  const { lang } = useLanguage();
  const labels = [
    { text: lang === 'PL' ? 'Działamy globalnie' : 'We operate globally' },
    { text: lang === 'PL' ? '500+ realizacji' : '500+ projects' },
    { text: lang === 'PL' ? 'Maksymalna konwersja' : 'Maximum conversion' },
    { text: lang === 'PL' ? 'Start projektu w 24h' : 'Project start in 24h' }
  ];
                ...
                <span className="flex flex-col items-start">
                      ...
                      >
                        {lang === 'PL' ? (
                          <>
                            ...
                            BUDŻETY, KTÓRE WRACAJĄ / POZYCJE, KTÓRE DOMINUJĄ.
                          </>
                        ) : (
                          <>
                            WEBSITES & E-COMMERCE STORES / <br />
                            BUDGETS THAT RETURN / POSITIONS THAT DOMINATE.
                          </>
                        )}
                      </div>
                    </div>
                  ...
                  <div className="flex flex-col md:flex-row items-start md:items-center gap-6 mt-6 w-full">
                    <div className="flex-1 max-w-[45ch]">
                      ...
                      >
                        {lang === 'PL' ? 'Zaufaj ekspertom z Solid Agency. Wyskocz poza szereg oryginalnym designem, który zarabia.' : 'Trust the experts at Solid Agency. Stand out with original design that earns.'}
                      </div>
                    </div>
    ...
    <section className="relative w-full pt-24 pb-32 lg:pt-16 overflow-hidden z-0" id="o-nas">
      ...
      >
        <div className="flex flex-col lg:flex-row justify-between items-start gap-12 lg:gap-24">
          ...
          >
            {lang === 'PL' ? 'Unikalne strony internetowe i sklepy internetowe. Kampanie Google & Meta Ads, które skutecznie skalują Twój biznes.' : 'Unique websites and e-commerce stores. Google & Meta Ads campaigns that effectively scale your business.'}
          </div>
          ...
          >
            <p className="mb-8">
              {lang === 'PL' ? 'Zespół ekspertów budujący wysokiej klasy strony internetowe i platformy e-commerce dla firm z całego świata.' : 'A team of experts building high-end websites and e-commerce platforms for companies worldwide.'}
            </p>
            <p>
              {lang === 'PL' ? 'Nasza praca łączy profesjonalny audyt UX, unikalne projektowanie UI i zaawansowany development, by wdrażać szybkie strony i dochodowe sklepy internetowe.' : 'Our work combines professional UX audit, unique UI design and advanced development to implement fast websites and profitable e-commerce stores.'}
            </p>
          </div>
        </div>
      </motion.div>
          ...
          <div className="w-full lg:w-[35%] text-center lg:text-left px-4 sm:px-0">
            ...
            >
              {lang === 'PL' ? (
                ...
                <>The future of your brand in <span className="text-shimmer font-semibold inline-block">48 hours.</span></>
              )}
            </h2>
            <p 
            ...
            >
              {lang === 'PL' ? 'Nie używamy szablonów, używamy doświadczenia. Nasz system pracy pozwala na wdrożenie unikalnych rozwiązań w tempie nieosiągalnym przy standardowych procesach. Dajemy Ci 48-godzinny przeskok od pomysłu do działającego, luksusowego interfejsu.' : 'We do not use templates, we use experience. Our work system allows for the implementation of unique solutions at a pace unattainable with standard processes. We give you a 48-hour leap from an idea to a working, luxurious interface.'}
            </p>
            ...
            <div className="mt-10 flex flex-col sm:flex-row items-center gap-6 lg:justify-end justify-center">
              ...
              >
                {lang === 'PL' ? 'Nie wierzysz? Tak to robimy.' : 'Don\'t believe it? This is how we do it.'}
              </p>
              <button className="px-6 py-3 rounded-full border border-white/20 hover:border-white/40 bg-white/5 hover:bg-white/10 text-white font-medium transition-all flex items-center gap-2 group text-sm sm:text-base">
                {lang === 'PL' ? 'Zobacz proces' : 'See the process'}
...
const getProjectsData = (lang: 'PL' | 'ENG') => [
  {
    ...
    location: 'USA',
    title: 'TAT2 SPIRITS',
    description: lang === 'PL' ? 'Nowa definicja nowoczesnego doświadczenia koktajlowego' : 'Redefining the Modern Cocktail Experience',
  ...
  },
  {
    ...
    location: 'UK',
    title: 'NEXUS TECH',
    description: lang === 'PL' ? 'Infrastruktura chmurowa nowej generacji dla biznesu' : 'Next-generation cloud infrastructure for business',
  ...
  },
  {
    ...
    location: 'POLAND',
    title: 'LUMINA',
    description: lang === 'PL' ? 'Platforma E-commerce dla luksusowego oświetlenia' : 'E-commerce Platform for Luxury Lighting',
  ...
  },
  {
    ...
    location: 'GERMANY',
    title: 'AURA MOBILITY',
    description: lang === 'PL' ? 'Aplikacja do współdzielenia miejskich pojazdów elektrycznych' : 'Urban Electric Vehicle Sharing App',
        ...
        <div className="sticky top-0 h-screen flex w-full overflow-hidden">
          ...
          <div className="w-[35%] h-full bg-black flex flex-col justify-between p-8 lg:p-12 xl:p-16 relative border-r border-white/10 z-20">
            <div className="flex flex-col items-start pt-10">
              ...
              <div className="text-white text-[clamp(1.25rem,2vw,1.5rem)] mt-4 font-inter-tight">{lang === 'PL' ? 'Nasze Projekty' : 'Our Projects'}</div>
            </div>
      ...
      <div className="md:hidden bg-black text-white pt-6 pb-20 px-4">
        <div className="flex flex-col gap-4">
          <div className="flex flex-col">
            ...
            >
              <span className="text-left">{lang === 'PL' ? 'NASZE' : 'OUR'}</span>
      ...
      <div className="relative z-[2] w-full px-0 sm:px-6 lg:px-24 xl:px-40">
        ...
        >
          ...
          {lang === 'PL' ? (
            ...
            <>WE CREATE<br className="sm:hidden" /><span className="hidden sm:inline"> </span>IT<br /><span className="whitespace-nowrap">FROM SCRATCH...</span></>
          )}
        </h2>
      </div>
    </section>
        ...
        >
          <span className="sm:hidden flex flex-col items-end text-right w-full">
            {lang === 'PL' ? (
              ...
              <span>WE CREATE<br />SUCCESS.</span>
            )}
          ...
          <span className="hidden sm:inline-grid text-left">
            ...
            <span className="col-start-1 row-start-1 flex flex-col w-full">
              <span className="text-left">{lang === 'PL' ? 'KREUJEMY' : 'WE CREATE'}</span>
              <span className="flex justify-between items-center w-full">
                ...
                <span className="text-right">{lang === 'PL' ? 'SUKCES.' : 'SUCCESS.'}</span>
              </span>
            </span>
          </span>
        </h2>
    ...
    <section className="bg-white py-5 lg:py-[1.25rem]">
      <div className="grid grid-cols-1 lg:grid-cols-2 px-6 lg:px-[6.25rem] gap-8 lg:gap-0">
        <div className="text-black w-full lg:max-w-[10.8125em]" style={{
          ...
          {lang === 'PL' ? 'Tworzymy cyfrowe doświadczenia, które budują prestiż i realnie zwiększają sprzedaż.' : 'We create digital experiences that build prestige and realistically increase sales.'}
        </div>
        <div className="text-black lg:col-start-2 lg:row-start-2 lg:ml-auto w-full lg:max-w-[27.5em]" style={{
          ...
          <p className="mb-4">
            {lang === 'PL' ? 'Za każdym projektem stoi prosty cel: tworzenie rozwiązań, które użytkownicy kochają, a firmy wykorzystują do wzrostu. Przez ostatnie 8 lat pomogliśmy ponad 200 klientom przekuć idee w nagradzane produkty i witryny, generujące konkretne zyski, w tym sześciocyfrowe przychody.' : 'Behind every project is a simple goal: creating solutions that users love and companies use to grow. Over the past 8 years, we have helped over 200 clients turn ideas into award-winning products and websites, generating concrete profits, including six-figure revenues.'}
          </p>
          <p>
            {lang === 'PL' ? 'Pracujemy z pełnym zaangażowaniem i dbałością o detale, pilnując najwyższej jakości na każdym etapie – od pierwszej rozmowy aż po finalne wdrożenie.' : 'We work with full commitment and attention to detail, ensuring the highest quality at every stage - from the first conversation to the final implementation.'}
          </p>
        </div>
      </div>
    </section>
    ...
    <section className="py-24 bg-white relative overflow-hidden" id="glass-boxes">
        ...
        >
          {boxes.map((box, idx) => (
            <div 
              key={idx} 
              className="flex-shrink-0 w-[85vw] sm:w-[500px] md:w-[600px] lg:w-[700px] snap-center relative rounded-3xl aspect-[4/3] sm:aspect-[3/2] lg:aspect-[16/9] overflow-hidden flex flex-col group transition-all duration-500 bg-[#D9D2E9] p-6 sm:p-10 lg:p-12"
            >
    ...
    <section className="py-24 bg-white relative overflow-hidden" id="latest-news">
      <div className="max-w-[100rem] mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <h2 className="text-[clamp(3rem,8vw,5.5rem)] font-inter-tight font-normal text-black mb-12 tracking-tight">
          {lang === 'PL' ? 'Nasze Informacje' : 'Our News'}
        </h2>
        ...
        <div ref={scrollContainerRef} className="flex overflow-x-auto lg:grid lg:grid-cols-4 gap-4 lg:gap-6 mb-12 pb-4 snap-x snap-mandatory scrollbar-hide -mx-4 px-4 sm:mx-0 sm:px-0">
            {news.map((item, idx) => (
              <a key={idx} href={item.link} onClick={(e) => { e.preventDefault(); onArticleClick(item.id); }} className="bg-[#0a0a0a] rounded-3xl overflow-hidden flex flex-col aspect-[3/4] group cursor-pointer no-underline min-w-[85vw] sm:min-w-[400px] lg:min-w-0 snap-center">
                <div className={`h-[60%] w-full overflow-hidden relative ${(item as any).padding || 'p-8'}`} style={{ backgroundColor: (item as any).bgColor || '#2B2B2B' }}>
                  <img 
                    src={item.image} 
                    alt={item.title}
                ...
                <div className="h-[40%] p-5 lg:p-6 flex flex-col justify-between">
                <h3 
                  className="text-white font-inter-tight leading-[1.3] font-light"
                  style={{ fontSize: 'clamp(1rem, 0.8rem + 1vw, 1.25rem)' }}
                >
                  {item.title}
                </h3>
                <div className="mt-4">
...
function ContactSection({ onStartProjectClick, onContactClick, onAskAwayClick }: { onStartProjectClick: () => void, onContactClick: () => void, onAskAwayClick: () => void }) {
...
const ContactPage = ({ onBack, onStartProjectClick, onAskAwayClick }: { onBack: () => void, onStartProjectClick: () => void, onAskAwayClick: () => void }) => {
  const { lang, t } = useLanguage();
  const text = lang === 'PL' ? "SKONTAKTUJ SIĘ SKONTAKTUJ SIĘ SKONTAKTUJ SIĘ SKONTAKTUJ SIĘ SKONTAKTUJ SIĘ SKONTAKTUJ SIĘ " : "CONTACT US CONTACT US CONTACT US CONTACT US CONTACT US CONTACT US ";
...
export default function App() {
  const { lang } = useLanguage();
  const [currentView, setCurrentView] = useState<'home' | 'contact' | 'start-project' | 'websites-offer' | 'onepage-offer' | 'ecommerce-offer' | 'ads-offer' | 'seo-offer' | 'poradnik' | 'article'>('home');
  ...
  return (
    <div className="relative bg-[#f8fafc] font-sans text-slate-900 selection:bg-[#8d32fc]/20 selection:text-[#8d32fc]">
      <Navbar 
        ...
        onAboutClick={() => handleScrollTo('o-nas')}
        onWorkClick={() => handleScrollTo('portfolio')}
      />
      {currentView === 'home' && (
        <main className="relative">
          ...
          <div className="relative z-10">
            ...
            <LatestProjects />
            <EightYearsSection />
            <FreeProjectOffer onContactClick={() => handleOpenContactModal()} />
            <ValueProps />
            <GlassBoxes 
      ...
      {currentView === 'contact' && (
        <ContactPage onBack={() => handleNavigate('home')} onStartProjectClick={() => handleNavigate('start-project')} onAskAwayClick={handleAskAwayClick} />
      )}
      ...
      {currentView === 'websites-offer' && (
        <WebsitesOfferPage 
          ...
          onContactClick={() => handleOpenContactModal(
            ...
            lang === 'PL' ? 'Otrzymaj wycenę swojego projektu w 30 minut.' : 'Get a quote for your project in 30 minutes.'
          )} 
          onAskAwayClick={handleAskAwayClick} 
          onOnePageOfferClick={() => handleNavigate('onepage-offer')} 
        />
      )}
      {currentView === 'onepage-offer' && (
        <OnePageOfferPage onBack={() => handleNavigate('websites-offer')} onContactClick={handleContactClick} onAskAwayClick={handleAskAwayClick} />
      )}
      {currentView === 'ecommerce-offer' && (
        <EcommerceOfferPage onBack={() => handleNavigate('home')} onContactClick={handleContactClick} onAskAwayClick={handleAskAwayClick} />
      )}
      {currentView === 'ads-offer' && (
        <AdsOfferPage onBack={() => handleNavigate('home')} onContactClick={handleContactClick} onAskAwayClick={handleAskAwayClick} />
      )}
      {currentView === 'seo-offer' && (
        <SeoOfferPage onBack={() => handleNavigate('home')} onContactClick={handleContactClick} onAskAwayClick={handleAskAwayClick} />
      )}
      {currentView === 'poradnik' && (
        <PoradnikPage onBack={() => handleNavigate('home')} onArticleClick={handleArticleClick} />
      )}
      {currentView === 'article' && (
        <ArticlePage onContactClick={handleContactClick} onAskAwayClick={handleAskAwayClick} articleId={selectedArticleId} onArticleClick={handleArticleClick} />
      )}
