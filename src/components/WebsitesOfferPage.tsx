...
export function LatestProjects({ projectsData, onContactClick }: { projectsData: ProjectData[], onContactClick: () => void }) {
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-y-20 sm:gap-y-24 lg:gap-y-32 gap-x-12 lg:gap-x-24">
          {projectsData.map((project) => (
            <motion.div 
              ...
              className="flex flex-col gap-4 sm:gap-6"
            >
              <h3 className="text-white text-[clamp(2.5rem,3vw,4rem)] font-inter-tight font-light leading-[1.05] tracking-tight lg:min-h-[8rem] lg:flex lg:items-end">
                {project.title}
              </h3>
              <div className="text-[#F3DEFF] text-xs sm:text-sm tracking-[0.2em] uppercase font-bold lg:min-h-[3rem] lg:flex lg:items-center">
                {project.deliveryTime}
              </div>
              <p className="text-white/80 text-[1rem] sm:text-lg lg:text-xl font-inter-tight font-light leading-relaxed lg:min-h-[12rem]">
                {project.description}
              </p>
              {project.points && (
                <ul className="flex flex-col gap-2 mt-2 flex-grow">
                  {project.points.map((point, idx) => (
                    <li key={idx} className="flex items-center gap-3 text-white/60 text-[0.9rem] sm:text-base font-inter-tight font-light">
...
export function WebsitesOfferPage({ onBack, onContactClick, onAskAwayClick, onOnePageOfferClick, onStartProjectClick }: { onBack: () => void; onContactClick: () => void; onAskAwayClick: () => void; onOnePageOfferClick: () => void; onStartProjectClick?: () => void }) {
  const { lang } = useLanguage();
  ...
  return (
    <div className="relative z-40 bg-black text-white min-h-screen">
      ...
      <LatestProjects projectsData={getProjectsData(lang)} onContactClick={onContactClick} />
