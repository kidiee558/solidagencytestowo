...
export function OnePageOfferPage({ onBack, onContactClick, onAskAwayClick }: { onBack: () => void; onContactClick: () => void; onAskAwayClick: () => void }) {
        ...
        <div>
          ...
          >
            {[1, 2, 3, 4].map((i) => (
              ...
              >
                <ProjectImage 
                  src={`https://picsum.photos/seed/onepage${i}/1200/1200`}
                  alt={`Project ${i}`}
                />
      ...
      <section className="min-h-screen bg-black text-white flex flex-col items-center justify-between p-6 relative overflow-hidden">
        ...
        <div className="relative z-20 flex flex-col items-center text-center mt-auto mb-auto px-6 translate-y-20 sm:translate-y-0">
          ...
          <div className="flex flex-col sm:flex-row gap-4 w-full sm:w-auto">
            <button onClick={onContactClick} className="rounded-full border border-white/20 hover:bg-white/5 transition-colors w-full sm:w-auto min-w-[12.5rem] h-[3.625rem] flex items-center justify-center">{lang === 'PL' ? 'Wystartuj teraz' : 'Start now'}</button>
